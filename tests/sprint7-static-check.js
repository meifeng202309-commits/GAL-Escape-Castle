...
const provenance=fs.readFileSync(new URL("../database/045_sprint7_teacher_intervention_provenance.sql",import.meta.url),"utf8").toLowerCase();
...
for(const token of ["s7_mirror_teacher_discussion_event","vote_opened_by_teacher","teacher_added_time","teacher_intervention_s5_vote_opened","teacher_intervention_s5_time_added","'teacher_override','valid',false"]){assert(provenance.includes(token),`missing intervention provenance: ${token}`);}
assert(!provenance.includes("status='completed'"),"Intervention provenance must not finalize a game run.");
