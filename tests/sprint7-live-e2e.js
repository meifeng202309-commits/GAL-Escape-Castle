...
async function fixture(mode,initialize=true){const room=unique("S7"),teacher=unique("T"),joins=[unique("G"),unique("A"),unique("L")];await rpc("s1_create_room",{p_room_code:room,p_teacher_token:teacher,p_gitte_join_code:joins[0],p_anna_join_code:joins[1],p_linda_join_code:joins[2]});const players=await Promise.all(joins.map(p_join_code=>rpc("s1_join_player",{p_room_code:room,p_join_code})));await rpc("s2_start_run",{p_room_code:room,p_teacher_token:teacher,p_run_mode:mode});if(initialize)await rpc("s3b_initialize_flow",{p_room_code:room,p_teacher_token:teacher});return{room,teacher,players}}
...
async function main(){
  const generic=await fixture("audit",false);
  await rpc("s2_open_discussion",{...teacher(generic),p_topic:"Teacher provenance",p_discussion_time_limit_sec:90,p_vote_time_limit_sec:60,p_show_initial_choices:false,p_require_final_vote:true,p_vote_options:[{id:"a",label:"A"},{id:"b",label:"B"}],p_tie_policy:"REPEAT_UNTIL_MAJORITY",p_revote_window_sec:30,p_max_revotes:1,p_fallback_resolution:null,p_silent_texting_mode:false});
  await rpc("s2_open_vote",teacher(generic));await rpc("s2_add_time",{...teacher(generic),p_seconds:30});let genericState=await rpc("s7_get_teacher_console",teacher(generic));
  assert(genericState.teacher_interventions.some(e=>e.event_type==="teacher_intervention_vote_opened_by_teacher")&&genericState.teacher_interventions.some(e=>e.event_type==="teacher_intervention_teacher_added_time"),"generic Teacher mutations missing durable intervention provenance");
