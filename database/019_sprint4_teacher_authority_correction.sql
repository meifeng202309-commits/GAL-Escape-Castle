-- Sprint 4 correction: use hashed Teacher authority for Asset Manager operations.
create table if not exists public.asset_registry_projection(
...
create unique index if not exists asset_candidates_one_active on public.asset_candidates(asset_key) where status='ACTIVE';
create table if not exists public.asset_events(event_id uuid primary key default gen_random_uuid(),asset_key text not null,event_type text not null,version int,details jsonb not null default '{}',created_at timestamptz not null default now());
...
create or replace function public.asset_manager_register_candidate(p_teacher_token text,p_candidate jsonb)
returns jsonb language plpgsql security definer set search_path=public as $$
...
create or replace function public.asset_manager_review(p_teacher_token text,p_asset_id uuid,p_decision text,p_reviewer text)
returns jsonb language plpgsql security definer set search_path=public as $$
declare c public.asset_candidates%rowtype;
begin if not exists(select 1 from public.s1_rooms where teacher_token_hash=public.s1_hash_token(p_teacher_token)) then raise exception 'Invalid teacher authority.';end if;
...
create or replace function public.asset_manager_mark_published(p_teacher_token text,p_asset_id uuid,p_storage_path text,p_sha256 text)
returns jsonb language plpgsql security definer set search_path=public as $$ declare c public.asset_candidates%rowtype;
begin if not exists(select 1 from public.s1_rooms where teacher_token_hash=public.s1_hash_token(p_teacher_token)) then raise exception 'Invalid teacher authority.';end if;select * into c from public.asset_candidates where asset_id=p_asset_id for update;
...
create or replace function public.asset_manager_activate(p_teacher_token text,p_asset_id uuid)
...
create or replace function public.asset_resolve(p_asset_key text) returns jsonb language plpgsql security definer set search_path=public as $$ declare r public.asset_registry_projection%rowtype;c public.asset_candidates%rowtype;
...
create or replace function public.asset_manager_state(p_teacher_token text) returns jsonb language plpgsql security definer set search_path=public as $$
begin if not exists(select 1 from public.s1_rooms where teacher_token_hash=public.s1_hash_token(p_teacher_token)) then raise exception 'Invalid teacher authority.';end if;return jsonb_build_object('assets',(select coalesce(jsonb_agg(to_jsonb(r)||jsonb_build_object('candidates',(select coalesce(jsonb_agg(to_jsonb(c) order by version desc),'[]') from public.asset_candidates c where c.asset_key=r.asset_key)) order by display_name),'[]') from public.asset_registry_projection r));end$$;
