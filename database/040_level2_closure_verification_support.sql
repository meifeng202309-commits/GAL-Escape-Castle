-- Teacher-authenticated deadline acceleration for NORMAL-path closure verification.
begin;

create function public.s6_verify_expire_discussion(
 p_room_code text,
...
begin
 ...
 where discussion_session_id=p_expected_discussion_session_id and run_id=g.run_id and scene_id='sprint6' and status='discussion'
 for update;
 if not found then raise exception 'Current Sprint 6 discussion was not found.'; end if;
...
grant execute on function public.s6_verify_expire_discussion(text,text,uuid) to anon,authenticated;
