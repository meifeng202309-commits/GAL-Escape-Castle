...
alter function public.s3b_get_player_state(text,text) rename to s3b_get_player_state_pre011;
...
create or replace function public.s3b_apply_meeting_resolution(p_room_code text,p_session_token text) returns jsonb language plpgsql security definer set search_path=public as $$ declare r text:=upper(trim(p_room_code));g public.game_runs%rowtype;result jsonb;route text; begin perform public.s1_get_player_by_session(r,p_session_token);g:=public.s2_get_active_run(r);perform 1 from public.game_runs where run_id=g.run_id for update;if not exists(select 1 from public.s3_runtime_scene_state where run_id=g.run_id and scene_id='act2_first_contact' and phase_key='meeting_discussion') or exists(select 1 from public.s3b_run_state where run_id=g.run_id and final_meeting_result is not null) then raise exception 'Meeting-resolution transition is unavailable.';end if;result:=public.s3b_apply_meeting_resolution_pre011(p_room_code,p_session_token);route:=result->>'final_meeting_result';perform public.s3b_set_scene(g.run_id,'act2_route_update','route_update',route,'CRITICAL_INFO','act02.032');return result||jsonb_build_object('route_update_pending',true);end;$$;

create or replace function public.s3b_ack_route_update(p_room_code text,p_session_token text)
returns jsonb language plpgsql security definer set search_path=public as $$
declare r text:=upper(trim(p_room_code));g public.game_runs%rowtype;route text;k text;
begin
  perform public.s1_get_player_by_session(r,p_session_token);g:=public.s2_get_active_run(r);perform 1 from public.game_runs where run_id=g.run_id for update;
  if not exists(select 1 from public.s3_runtime_scene_state where run_id=g.run_id and scene_id='act2_route_update' and phase_key='route_update') then raise exception 'Route-update transition is unavailable.';end if;
  select final_meeting_result into route from public.s3b_run_state where run_id=g.run_id for update;
  k:=case route when 'library' then 'act03.001' when 'great_hall' then 'act02.038' when 'main_gate' then 'act02.043' when 'west_tower' then 'act02.046' else 'act02.049' end;
  perform public.s3b_set_scene(g.run_id,'act2_rendezvous','route_consequence',route,'CINEMATIC_MESSAGE',k);
  return jsonb_build_object('ok',true,'route',route);
end; $$;
...
returns jsonb language plpgsql security definer set search_path=public as $$
declare r text:=upper(trim(p_room_code));p public.s1_room_players%rowtype;g public.game_runs%rowtype;base jsonb;ready boolean;
begin
  p:=public.s1_get_player_by_session(r,p_session_token);g:=public.s2_get_active_run(r);base:=public.s3b_get_player_state_pre011(p_room_code,p_session_token);
  select count(*)=3 into ready from public.s3b_player_progress where run_id=g.run_id and first_meeting_locked_at is not null and grab_complete and left_start_room;
  return jsonb_set(base,'{queued_first_messages}',case when ready then (select coalesce(jsonb_agg(jsonb_build_object('display_name',rp.display_name,'choice_id',pp.first_meeting_choice,'template_text_key',case when pp.first_meeting_choice='help'then'act02.011'else'act02.010'end,'location_text_key',case pp.first_meeting_choice when'library'then'act01-g.009'when'great_hall'then'act01-g.010'when'main_gate'then'act01-g.011'when'west_tower'then'act01-g.012'when'chapel'then'act01-g.013'else null end)order by rp.role_slot),'[]')from public.s3b_player_progress pp join public.s1_room_players rp using(player_id)where pp.run_id=g.run_id)else'[]'::jsonb end);
end; $$;

grant execute on function public.s3b_initialize_flow(text,text),public.s3b_ack_act1_opening(text,text),public.s3b_submit_act1_choice(text,text,text),public.s3b_complete_act1(text,text),public.s3b_submit_first_meeting(text,text,text),public.s3b_grab(text,text),public.s3b_leave_start_room(text,text),public.s3b_apply_meeting_resolution(text,text),public.s3b_ack_route_update(text,text),public.s3b_complete_foldback(text,text),public.s3b_follow_sign(text,text),public.s3b_submit_library_code(text,text,text),public.s3b_submit_act4_choice(text,text,text),public.s3b_apply_act5_resolution(text,text),public.s3b_choose_post_inspection_route(text,text,text),public.s3b_get_player_state(text,text) to anon,authenticated;
