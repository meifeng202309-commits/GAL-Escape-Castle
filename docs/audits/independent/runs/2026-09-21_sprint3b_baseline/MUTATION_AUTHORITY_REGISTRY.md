...
# C5 — Replay / Stale Request Review

C5 distinguishes:
- **duplicate/replay**: the same logical action is submitted again;
- **stale**: a request was created for an earlier interaction state but arrives after the server has moved on;
- **post-terminal**: old mutation arrives after current flow has terminated;
- **cross-session**: a token/request from another room/session attempts to mutate the current run.

## C5.1 Cross-session baseline

Player authentication is room + session-token bound.  
Teacher authentication is room + Teacher-token bound.

A valid token from one room cannot be re-targeted to another room merely by changing `p_room_code`.

After Teacher release:
- old player token no longer matches `session_token_hash`;
- stale old-browser mutations fail authentication.

No new cross-room/cross-session defect was found in C5.

## C5.2 Locked behavior choices / votes

Sprint3B private choices are generally replay-safe through:
- per-player NULL/locked predicates;
- scene/phase guards;
- serialization trigger.

Examples:
- ACT1 choice replay rejected;
- first-meeting replay rejected;
- ACT4 choice replay rejected;
- route ACK replay rejected;
- FOLLOW SIGN replay rejected;
- post-terminal route replay rejected.

Sprint2 vote rows have a unique decision identity and duplicate vote in the same selected vote round is rejected.

However, the request itself does **not identify the vote round/session the client intended**. This creates the stale-retarget defect below.

## C5.3 IDA-009 — stale DiscussionRoom commands can be retargeted to a newer round/session

Player message API:

`s2_send_message(room_code, session_token, message_text)`

does not accept:
- `discussion_session_id`;
- `vote_round`;
- scene/phase/step expected by the client.

At execution time it selects:

> latest `discussion_sessions` row for the run, ordered by `vote_round desc`.

Final `s2_submit_vote(room_code, session_token, choice_id)` does the same.

Therefore a stale UI/in-flight request is not validated against the interaction in which the student actually made the action.

### Deterministic stale-message example

1. Player UI is still displaying DiscussionRoom A.
2. Server advances/creates DiscussionRoom B with a higher vote_round.
3. A delayed message request originating from A reaches server.
4. Server selects B because B is now latest.
5. The message is persisted with B's `discussion_session_id / scene_id / phase_key / step_key`.

The student's real behavior has been reclassified into a different interaction.

### Deterministic stale-vote example

Most direct case: re-vote.

1. Round N uses options X/Y/Z.
2. 1:1:1 creates Round N+1 with the same options.
3. a delayed vote request generated by stale Round N UI arrives after Round N+1 has entered voting.
4. `s2_submit_vote` selects Round N+1.
5. the old request can become the player's locked vote for the new round.

This violates the principle that a stale old-phase mutation must be rejected rather than reinterpreted.

Recorded as **IDA-009 HIGH**.

## C5.4 IDA-010 — response-loss retry can duplicate a dialogue behavior event

`dialogue_messages` has:
- generated `message_id`;
- no client/request idempotency key;
- no uniqueness constraint representing one user submission.

`s2_send_message` inserts unconditionally once current discussion is open.

Client `sendMessage()`:
- sends text;
- clears input only after successful response;
- on error leaves the text and re-enables Send.

Failure window:

1. server inserts message and event;
2. HTTP response is lost;
3. client sees error and leaves original text available;
4. retry inserts another message/event;
5. persisted history contains two apparently genuine player messages.

Because later behavior analysis may use:
- message count;
- initiative;
- information sharing;
- timing/order;

the database cannot distinguish retry duplication from two intentional identical messages.

Recorded as **IDA-010 HIGH**.

## C5.5 IDA-011 — response-loss retry can double-count a wrong Library attempt

`s3b_submit_library_code` has no client/request idempotency identity.

For an incorrect attempt:
- puzzle remains in `library_box`;
- `puzzle_attempt_number` increments;
- `puzzle_hint_stage` may advance;
- a `s3b_library_attempts` row is inserted.

Failure window:

1. wrong attempt commits;
2. response is lost;
3. player retries the same visible submission;
4. scene is still library_box and request remains valid;
5. second call creates another attempt number.

Consequences:
- one real player action is recorded as two attempts;
- hints can be revealed earlier than intended;
- the attempt ledger no longer represents actual player actions.

A correct attempt is safer because the first commit resolves/advances the scene and replay is rejected.

Recorded as **IDA-011 MEDIUM**, because Library attempts are Game Track rather than Behavior Track, but the defect changes gameplay and evidence.

## C5.6 Other replay behavior

### SHARE PHOTO
The shared-photo uniqueness key plus ON CONFLICT behavior makes same sender/recipient/item/view replay effectively idempotent at row-count level.

### Item view
Re-sending the already-current view returns success without another transition.

### Formal flow initialization
Replay is rejected once `s3b_run_state` exists; state can be recovered by getter.

### Group progression resolvers
Meeting apply / fold-back / ACT5 apply / post-inspection resolution are protected by scene/result changes and generally reject replay after successful advancement.

### Teacher add-time
`s2_add_time` is intentionally additive and has no request idempotency identity. A response-loss retry can add time twice.

C5 does not open a separate finding because repeated Add Time is also a legitimate deliberate Teacher action, and this does not fabricate player behavior. It should nevertheless be included in later failure-injection testing.

### Session release / legacy reset
Repeated calls can create repeated Teacher events, but they do not duplicate formal player behavior. Lower priority; no separate C5 finding.

## C5.7 C5 conclusion

New confirmed findings:
- IDA-009 HIGH — stale DiscussionRoom actions retarget to latest session/round.
- IDA-010 HIGH — response-loss retry duplicates dialogue behavior.
- IDA-011 MEDIUM — response-loss retry duplicates wrong Library attempts.
