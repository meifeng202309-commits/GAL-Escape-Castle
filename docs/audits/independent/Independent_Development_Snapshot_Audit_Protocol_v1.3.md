# GAL Escape Castle — Independent Development Snapshot Audit Protocol v1.3
...
Status: ACTIVE  
Supersedes for future execution: Independent_Development_Snapshot_Audit_Protocol_v1.2.md  
Version: 1.3
...
# 5A. Mandatory Codex recurring-error focus

Every independent audit must additionally apply:

`docs/onboarding/GAL_ESCAPE_CASTLE_CA_CODING_AUDIT_RULES_V1.1.md`

Specifically, the auditor must explicitly test the six recurring CD/Codex failure patterns:

A. local correctness but broken cross-module handoff;  
B. happy-path assumptions under response loss / stale request / retry / concurrency;  
C. UI-only rule without server-side invariant;  
D. current state preserved but historical evidence lost;  
E. authority accretion across legacy/generic/new layers;  
F. self-confirming tests that share the implementation's assumptions.

These are audit lenses, not replacements for Methods 1–9.

The run-specific rundown must include one consolidated recurring-pattern review before final disposition.

---

# 5B. Construction / audit independence

Independent audit must preserve partial independence between CD construction reasoning and CA falsification reasoning.

Rules:
- share canonical requirements and invariants;
- do not pre-prescribe CD implementation mechanics;
- do not pre-publish CA's exact adversarial attack plan;
- when auditing completed CD work, first derive the implemented model from code/migrations/tests;
- defer detailed CD implementation rationale / claimed protections until after the first-pass model and initial failure hypotheses are formed, when practical.

Audit-request metadata such as scope, commits, changed files, test commands/results and known limitations may be read immediately.

Purpose:
- reduce correlated blind spots;
- prevent CA from merely checking whether CD implemented CA's own suggested design;
- create genuine construction-vs-falsification cross-checking.
...
# Conditional Pre-Approval Gate

Execute this gate only if the independent audit is otherwise ready to issue PASS / READY.

## P0. Independence check

Before forecasting next-scope risks, confirm:
- CA will identify risk areas and invariants only;
- CA will not prescribe implementation mechanisms;
- CA will not pre-publish its exact future adversarial attack plan;
- future re-audit will begin from code-first independent reconstruction.

## P1. Next-Scope Failure Forecast

Identify the next authorized CD work and forecast 3–8 concrete likely failure modes using:
- recurring Patterns A–F;
- architecture/interfaces touched next;
- unresolved NOT VERIFIED boundaries;
- adjacent findings/lessons.

Each forecast item should say:
- where the risk is;
- why it is high-risk;
- which invariant/failure class deserves attention.

It must not say:
- which exact schema/field/lock/algorithm to use;
- how CD should structure the implementation;
- which exact adversarial test CA will later run.

If the forecast exposes an already-existing baseline defect or missing required authority/interface contract, reopen the current audit instead of approving.

If the next scope is not canonicalized, record that fact rather than inventing work.

## P2. Risk-only handoff

The final CA→CD approval handoff must include:
...
This section is mandatory for an approval handoff and omitted/N/A for FAIL/BLOCKED unless the auditor has a concrete reason to include it.
