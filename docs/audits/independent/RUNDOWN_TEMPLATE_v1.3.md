# Independent Audit RUNDOWN Template v1.3
...
- confirm risk forecast will contain risk areas + invariants/failure classes only;
- do not prescribe implementation mechanics;
- do not disclose exact future adversarial attack plan;
- record that future re-audit begins with code-first independent reconstruction.
...
- forecast 3–8 concrete high-risk areas using Patterns A–F and next-scope interfaces;
- state why each is high-risk;
- state invariant/failure class to watch;
- if forecast exposes a current-baseline defect, reopen current audit.
If current disposition is FAIL/BLOCKED:
- mark [N/A] with reason.

### [ ] P2 Risk-only CD handoff
If approving:
- use the four-column risk-only table from Protocol v1.3;
- do not include required schema/lock/algorithm/test recipe.
If not approving:
- mark [N/A] with reason.
...
Output: FINDINGS.md
Requirement: exact full table schema from Protocol v1.3.
