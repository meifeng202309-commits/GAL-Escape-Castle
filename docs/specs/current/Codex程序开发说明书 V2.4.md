...
> **V2.4 governance-hardening revision:** supersedes V2.3 and adds explicit canonical-ownership/write-protection rules after the Sprint6 localization authority incident. V2.3/V2.1/V2.0 are no longer current source-of-truth files.
...
3. `docs/specs/current/Codex程序开发说明书 V2.4.md`（本文件）

Repository hygiene：
- V4.0 / V2.4 是当前source-of-truth；current specs统一放在 `docs/specs/current/`；
...
### 4.4.1 Canonical ownership / CD write-protection — HARD STOP

实现需要某个 canonical 内容，**不等于 CD 自动获得定义或修改该 canonical 内容的 authority**。

对任何由其他角色维护的 canonical source：

- CD只能消费已经 canonicalized / approved 的内容；
- 除非当前 governance 明确把该文件的写权限委托给CD，否则CD不得为了“让实现闭环”而自行新增、删除、改写或批准 canonical 内容；
- “我发现缺口”与“我有权填补缺口”是两件不同的事。

对 localization，以下文件对CD属于 **WRITE-PROTECTED CANONICAL SOURCE**：
...
- 自行新增 `text_key`；
- 自行删除 canonical row；
- 自行撰写或改写 English master / Nederlands / 中文；
- 把自己提出的临时文字直接写入该CSV并当成 approved canonical；
- 先在代码中消费一个未批准的新key，再要求GA/Teacher事后追认。

如果实现发现缺少GAL-facing `text_key`：

```text
STOP canonical wording work
→ CD sends ACTION_REQUIRED to GA
→ describe semantic purpose + scene/phase + UI context
→ GA/Teacher decides wording
→ GA updates canonical CSV in a GA-owned canonical commit
→ GA sends commit SHA / handoff
→ CD consumes that approved key in a separate implementation commit
→ CD regenerates any derived runtime localization
→ CA audits
```

Commit / provenance rule：

- canonical-source change 与 CD implementation consumption **必须分开 commit**；
- GitHub账号的 author/committer 身份不能替代 Agent authority provenance；
- authority evidence以 owner-role Action Log + inter-Agent handoff + canonical commit 为准；
- CD audit request不得把一个未经owner批准的 canonical-source modification 混在 implementation diff 中；
- derived JS/JSON/runtime representation仍可由CD从 approved canonical source生成，但不得反向成为人工 source of truth。

如果CD认为 canonical source 本身必须改变，但没有明确 authority：

> **STOP → report CANONICAL GAP / SPEC CONFLICT → request owner clarification.**
...
V2.4不要求在 Sprint 2 重做 authentication。
...
# 21. Definition of Done — Codex Guide V2.4
