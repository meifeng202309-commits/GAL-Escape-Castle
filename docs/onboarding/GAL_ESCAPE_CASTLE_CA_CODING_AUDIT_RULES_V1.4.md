# GAL Escape Castle — CA Coding Audit Rules V1.4

Project: GAL Escape Castle  
Owner: CA — Coding Audit Agent  
Status: ACTIVE  
Supersedes: GAL_ESCAPE_CASTLE_CA_CODING_AUDIT_RULES_V1.3.md  
Purpose: mandatory rules for normal CD→CA coding audits and supplemental rules for independent audits. V1.4 adds a mandatory canonical-ownership check and protected-source provenance gate.
...
- Did the implementation interval modify a canonical source owned by another role?
- If so, is there prior owner-role canonical approval and separate provenance?
...
# 2A. Mandatory Canonical Ownership Check

This check is mandatory in **every substantive CD audit**, independent of Patterns A–F.

CA must inspect the changed-file set for the submitted implementation interval and ask:

1. Did CD modify any canonical source whose maintenance authority belongs to GA, VA, CA governance, Teacher, or another explicitly named owner?
2. If yes, does a **prior/separate owner-authorized canonical commit** exist?
3. Is that authority evidenced by the owner role's Action Log and/or a valid inter-Agent handoff?
4. Is the CD implementation commit consuming that approved canonical state rather than creating it?

For the localization catalog:

```text
docs/specs/current/localization/GAL_Castle_Escape_Text_Catalog_V1.0.csv
```

any new/deleted/changed GAL-facing row in a CD implementation interval requires GA/Teacher canonical authority evidence.

If protected canonical content was authored or changed by CD without valid prior owner approval:

```text
BLOCKED — CANONICAL AUTHORITY VIOLATION
```

Content quality does not cure the authority violation. A translation can be linguistically correct and still be non-canonical because the wrong role authored/approved it.

Required remediation pattern:

```text
owner role canonicalizes
→ owner canonical commit
→ owner handoff / Action Log evidence
→ CD consumes in a separate implementation commit
→ CA re-audits
```

GitHub `author` / `committer` identity is **not sufficient provenance**, because project Agents may share the same repository account. Use role Action Logs, inter-Agent messages, and commit boundaries.

CA must not ratify missing gameplay/wording/visual canon itself merely to unblock implementation. Route the gap to the canonical owner.
...
State whether any protected/role-owned canonical source changed in the implementation interval, and cite the owner-authority evidence when applicable. If none changed, say so explicitly.
