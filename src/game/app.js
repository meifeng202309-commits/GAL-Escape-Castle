...
async function playSprint6Audio(key,asset){const url=sprint6AudioUrls.get(key)||`https://qdcbdcjobzytzhnhfwyn.supabase.co/storage/v1/object/public/${asset.storage_path}`,audio=new Audio(url);audio.preload="auto";audio.volume=key==="audio.wet_scraping"?0:sprint6AudioVolume();sprint6AudioHandles.set(key,audio);audio.addEventListener("ended",()=>sprint6AudioHandles.delete(key),{once:true});try{await audio.play();sprint6PendingAudio=null;if(key==="audio.wet_scraping")fadeSprint6Audio(audio,1500)}catch(error){sprint6PendingAudio={key,asset};console.warn("Sprint6 audio playback paused",key,error);sprint3bStatus.innerHTML=localizedHtml("runtime.audio.blocked")}}
function retryPendingSprint6Audio(){if(sprint6AudioArmed&&sprint6PendingAudio){const pending=sprint6PendingAudio;sprint6PendingAudio=null;playSprint6Audio(pending.key,pending.asset)}}
