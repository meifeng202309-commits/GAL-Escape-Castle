...
function sprint5EvidencePanel(pocket,phase){
  if(!pocket)return"";
  const itemHtml=(pocket.items||[]).map(item=>`<details class="evidence-item"><summary>${localizedHtml(item.name_text_key)}</summary><p class="muted">${escapeHtml(item.current_view||"")}</p>${item.item_key==="linda_stopped_watch"?`${localizedHtml("act01-l.002")}<button type="button" data-flip-item="${item.item_key}" data-target-view="${item.current_view==="back"?"front":"back"}">${localizedHtml("act01-g.028")}</button>`:""}</details>`).join("");
