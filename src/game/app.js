...
function renderSprint8(payload){setSprint6CinematicMode({phase_key:"act14_complete"});choiceArea.classList.add("hidden");revealArea.classList.add("hidden");discussionPanel.classList.add("hidden");sprint3bPanel.classList.remove("hidden");sprint3bActions.innerHTML="";sprint3bStatus.textContent="";if(sprint8RevealRunId===payload.run_id)return;sprint8RevealRunId=payload.run_id;clearTimeout(sprint8RevealTimer);let stage=0;const frames=[`<div class="s8-fade" aria-hidden="true"></div>`,`<p>${localizedHtml("act14.001")}</p>`,`<p>${localizedHtml("act14.002")}</p>`,`<strong>${localizedHtml("act14.003")}</strong>`,`<strong class="s8-separate-screen">${localizedHtml("act14.004")}</strong>`,`<p>${localizedHtml("act14.005")}</p>`],delays=[900,1100,1400,1800,1800];const advance=()=>{sprint3bText.innerHTML=`<section class="s8-ending s8-stage-${stage}">${frames[stage]}</section>`;if(stage<frames.length-1)sprint8RevealTimer=setTimeout(()=>{stage+=1;advance()},delays[stage])};advance()}
