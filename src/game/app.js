...
let sprint8RevealRunId = "";
let sprint8RevealTimer = null;
let sprint6AudioIdentity = "";
const sprint6AudioHandles = new Map();
const sprint6AudioUrls = new Map();
let sprint6AudioArmed = false;
let sprint6PendingAudio = null;
...
function sprint6AudioOutbox() {
  try {
    return JSON.parse(localStorage.getItem(SPRINT6_AUDIO_OUTBOX_KEY) || "{}");
  } catch {
    return {};
  }
}

function saveSprint6AudioOutbox(outbox) {
  localStorage.setItem(SPRINT6_AUDIO_OUTBOX_KEY, JSON.stringify(outbox));
}
...
document.addEventListener("pointerdown", armSprint6Audio, { once: true });
document.addEventListener("pointerdown", retryPendingSprint6Audio);
...
const S6_DOORS=[["red","act09.011"],["blue","act09.012"],["black","act09.013"]];
const S6_SCENE_ASSETS=new Set(["shared.great_hall","prop.golden_key","shared.main_gate","ending.castle_exterior"]);
const TRIAL_PLACEHOLDER_PATHS={"opening.gitte_room":"assets/staging/opening.gitte_room/v001/opening.gitte_room__v001.svg","opening.anna_room":"assets/staging/opening.anna_room/v001/opening.anna_room__v001.svg","opening.linda_study":"assets/staging/opening.linda_study/v001/opening.linda_study__v001.svg","ending.castle_exterior":"assets/staging/ending.castle_exterior/v001/ending.castle_exterior__v001.svg"};
...
function renderSprint3b(state) {
  ...
  const sceneAsset=scene.scene_id==="act1_wake_up"?({"GAL-A":"opening.gitte_room","GAL-B":"opening.anna_room","GAL-C":"opening.linda_study"}[session.role_slot]):(["act3_library","act4_known_unknown"].includes(scene.scene_id)?"shared.library":null);
  sprint3bText.innerHTML=(sceneAsset?`<img id="s3bSceneImage" class="s5-scene" alt="">`:"")+(scene.phase_key==="route_update"
  ...
  if(sceneAsset)setS5Asset("s3bSceneImage",sceneAsset);
...
function sprint5EvidencePanel(pocket,phase){
  if(!pocket)return"";
  ...
  return `<section class="notice evidence-panel" data-s5-evidence-phase="${escapeHtml(phase)}">${itemHtml}${groupHtml}${memoryHtml}${shareHtml}</section>`;
}
function renderSprint5(payload,pocket){
  ...
  const pocketHtml=sprint5EvidencePanel(pocket,s.phase_key);
  if(s.phase_key.startsWith("act6")||s.phase_key.startsWith("act7"))visual+=pocketHtml;
...
function setSprint6Volume(mode){sprint6VolumeMode=mode;localStorage.setItem("gal.s6.audio-mode",mode);for(const audio of sprint6AudioHandles.values())audio.volume=sprint6AudioVolume();document.querySelectorAll("[data-s6-volume]").forEach(button=>button.setAttribute("aria-pressed",String(button.dataset.s6Volume===mode)));retryPendingSprint6Audio()}
function fadeSprint6Audio(audio,duration){const started=performance.now();audio.volume=0;const timer=setInterval(()=>{const progress=Math.min(1,(performance.now()-started)/duration);audio.volume=sprint6AudioVolume()*progress;if(progress===1)clearInterval(timer)},50)}
async function flushSprint6AudioConsumptions(){if(!session)return;const outbox=sprint6AudioOutbox();for(const[occurrenceId,outcome]of Object.entries(outbox)){try{await rpc("s6_mark_audio_consumed",{p_room_code:session.room_code,p_session_token:session.session_token,p_occurrence_id:occurrenceId,p_outcome:outcome});delete outbox[occurrenceId];saveSprint6AudioOutbox(outbox)}catch(error){console.warn("Sprint6 audio consumption remains queued",occurrenceId,error)}}}
async function markSprint6AudioConsumed(occurrenceId,outcome="ended"){if(!occurrenceId)return;const outbox=sprint6AudioOutbox();outbox[occurrenceId]=outcome;saveSprint6AudioOutbox(outbox);await flushSprint6AudioConsumptions()}
async function playSprint6Audio(key,asset,occurrenceId){const url=sprint6AudioUrls.get(key)||`https://qdcbdcjobzytzhnhfwyn.supabase.co/storage/v1/object/public/${asset.storage_path}`,audio=new Audio(url);audio.preload="auto";audio.volume=key==="audio.wet_scraping"?0:sprint6AudioVolume();audio.dataset.occurrenceId=occurrenceId||"";sprint6AudioHandles.set(key,audio);audio.addEventListener("ended",()=>{sprint6AudioHandles.delete(key);markSprint6AudioConsumed(occurrenceId)},{once:true});try{await audio.play();sprint6PendingAudio=null;if(key==="audio.wet_scraping")fadeSprint6Audio(audio,1500)}catch(error){sprint6PendingAudio={key,asset,occurrenceId};console.warn("Sprint6 audio playback paused",key,error);sprint3bStatus.innerHTML=localizedHtml("runtime.audio.blocked")}}
function retryPendingSprint6Audio(){if(sprint6AudioArmed&&sprint6PendingAudio){const pending=sprint6PendingAudio;sprint6PendingAudio=null;playSprint6Audio(pending.key,pending.asset,pending.occurrenceId)}}
function setSprint6CinematicMode(state){const cinematic=state.phase_key==="act12_cinematic",stage=cinematic?state.cinematic_stage:0;document.body.classList.toggle("s6-fading",stage>=4&&stage<=9);document.body.classList.toggle("s6-blackout",stage===10);document.body.dataset.s6Stage=String(stage)}
function scheduleSprint6LeverCenter(payload){if(payload.station_b_stage!=="lever_held"||payload.station_task||sprint6LeverTimer)return;const body={p_room_code:session.room_code,p_session_token:session.session_token,...sprint6Identity(),p_task_value:"lever_center"},pending=requestIdentity("s6_complete_station_v2",`${body.p_expected_phase}.${body.p_expected_step}.${body.p_expected_round}.lever-center`,body);body.p_client_request_id=pending.requestId;sprint6LeverTimer=setTimeout(async()=>{sprint6LeverTimer=null;try{await rpc("s6_complete_station_v2",body);clearRequestIdentity(pending.key);await refreshState()}catch(error){sprint3bStatus.innerHTML=`<span class="bad">${escapeHtml(error.message)}</span>`}},1200)}
function sprint6AudioControls(){return `<div class="s6-audio-controls" role="group">${[["full","runtime.audio.full"],["reduced","runtime.audio.reduced"],["mute","runtime.audio.mute"]].map(([mode,key])=>`<button type="button" class="secondary" data-s6-volume="${mode}" aria-pressed="${sprint6VolumeMode===mode}">${localizedHtml(key)}</button>`).join("")}</div>`}
function sprint6Discussion(payload){const d=payload.discussion;if(!d)return"";const ready=Date.parse(d.phase_deadline)<=Date.now();return `<section class="discussion-room"><h3>${localizedHtml(d.phase_key==="act10_discussion"?"act10.004":d.phase_key==="act11_discussion"?"act11.001":"act09.004")}</h3><p class="muted">${escapeHtml(formatDeadline(d.phase_deadline,"discussion"))}</p><div class="transcript">${d.messages.length?d.messages.map(m=>`<article class="message"><b>${escapeHtml(m.display_name)}</b><p>${escapeHtml(m.message_text)}</p></article>`).join(""):localizedHtml("discussion.no_messages")}</div><form id="s6MessageForm" class="composer"><input id="s6MessageText" maxlength="1000" required><button>${localizedHtml("discussion.send_message")}</button></form><button data-s6-rpc="s6_close_discussion_v2" data-s6-discussion="${d.discussion_session_id}" ${ready?"":"disabled"}>${localizedHtml("common.004")}</button></section>`}
function renderSprint6(payload){currentSprint6State=payload;const s=payload.state;setSprint6CinematicMode(s);choiceArea.classList.add("hidden");revealArea.classList.add("hidden");sprint3bPanel.classList.remove("hidden");const black=s.phase_key==="act12_cinematic"&&s.cinematic_stage===10;const hall=s.act_no===9?`<div class="s6-door-layer"><span data-anchor="great_hall_red_door" class="s6-door red ${s.act9_step>=2?"open":""}">${localizedHtml("act09.001")}</span><span data-anchor="great_hall_blue_door" class="s6-door blue ${s.act9_step>=3?"available":""} ${s.act9_step>=4?"open":""}">${localizedHtml("act09.002")}</span><span data-anchor="great_hall_black_door" class="s6-door black">${localizedHtml("act09.003")}</span></div>`:"";let body=`<div class="s6-stage ${black?"blackout":""}">${black?"":`<p class="eyebrow">ACT ${s.act_no}</p>${payload.scene_asset_key?`<div class="s6-scene-wrap"><img id="s6SceneImage" class="s5-scene" alt="">${hall}</div>`:""}${payload.private_clue?`<div class="notice">${localizedHtml(payload.private_clue.text_key)}</div>`:""}${localizedKeys(s.feedback_text_keys||[])}`}</div>`,actions="";body+=sprint6AudioControls();if(payload.private_choices_revealed?.length)body+=`<div class="notice">${payload.private_choices_revealed.map(x=>`${escapeHtml(x.role_slot)}: ${localizedHtml(x.choice_id==="take"?"act10.004":"act10.005")}`).join("<br>")}</div>`;if(payload.discussion)actions=sprint6Discussion(payload);if(s.phase_key==="act9_console"){const choices=s.act9_step===1||s.act9_step===3?S6_DOORS:s.act9_step===2?[["enter_red","act09.015"],["do_not_enter","act09.016"]]:[["enter_blue","act09.020"],["stay","act09.021"]];actions=s5Buttons(choices,"s6_submit_group_choice_v2").replaceAll("data-s5-","data-s6-")}if(s.phase_key==="act10_private")actions=s5Buttons([["take","act10.004"],["leave","act10.005"]],"s6_submit_private_choice_v2").replaceAll("data-s5-","data-s6-");if(s.phase_key==="act10_final_vote")actions=s5Buttons([["take","act10.006"],["leave","act10.007"]],"s6_submit_group_choice_v2").replaceAll("data-s5-","data-s6-");if(s.phase_key==="act10_result")actions=`<button data-s6-rpc="s6_advance_v2">${localizedHtml("act10.010")}</button>`;if(s.phase_key==="act11_allocation")actions=s5Buttons((s.gold_key?[["A","act11.012"],["B","act11.013"],["WATCHER","act11.014"]]:[["A","act11.012"],["B","act11.013"],["C","act11.004"]]),"s6_submit_allocation_v2").replaceAll("data-s5-","data-s6-");if(s.phase_key==="act12_tasks"&&payload.allocation){const task={A:["act11.005","1897"],B:["act11.007",payload.station_b_stage==="lever_held"?"lever_center":"lever_hold"],C:["act11.008","silver_key_inserted"],WATCHER:["act11.015","corridor_watched"]}[payload.allocation];actions=payload.station_task?`<button data-s6-rpc="s6_engage_v2" data-s6-role="${escapeHtml(payload.allocation)}">${localizedHtml(payload.allocation==="WATCHER"?"act12.004":"act12.001")}</button>`:`${localizedHtml(task[0])}${payload.allocation==="B"&&payload.station_b_stage==="lever_held"?`<span class="s6-lever-track" aria-hidden="true"><span></span></span>`:""}${payload.allocation==="A"?`<form id="s6StationForm" class="composer"><input id="s6StationValue" inputmode="numeric" maxlength="4" required><button>${localizedHtml("act12.001")}</button></form>`:payload.allocation==="B"&&payload.station_b_stage==="lever_held"?"":`<button data-s6-rpc="s6_complete_station_v2" data-s6-value="${task[1]}">${localizedHtml(payload.allocation==="B"?"act12.002":payload.allocation==="C"?"act12.003":"act12.004")}</button>`}`}if(s.phase_key==="act12_pressure"){const failing=payload.allocation===s.mechanism_failure_station;actions=s5Buttons(failing?[["one","act12.008"],["two","act12.009"],["three","act12.010"],["four","act12.011"]]:[["hold","act12.012"],["encourage","act12.013"],["watch","act12.014"],["prepare","act12.015"]],"s6_submit_private_choice_v2").replaceAll("data-s5-","data-s6-")}if(s.phase_key==="act13_escape"&&Date.parse(payload.act13_continue_at)<=Date.now())actions=`<button data-s6-rpc="s6_advance_v2">${localizedHtml("common.004")}</button>`;if(s.phase_key==="act13_boundary")actions=`<button id="s8Finalize">${localizedHtml("common.004")}</button>`;sprint3bText.innerHTML=body;sprint3bActions.innerHTML=actions;hydrateSprint6(payload);sprint3bActions.querySelectorAll("[data-s6-rpc]").forEach(b=>b.addEventListener("click",()=>runSprint6Action(b)));document.getElementById("s8Finalize")?.addEventListener("click",finalizeSprint8);document.getElementById("s6MessageForm")?.addEventListener("submit",sendSprint6Message);document.getElementById("s6StationForm")?.addEventListener("submit",submitSprint6Station);document.querySelectorAll("[data-s6-volume]").forEach(button=>button.addEventListener("click",()=>setSprint6Volume(button.dataset.s6Volume)));scheduleSprint6LeverCenter(payload)}

async function finalizeSprint8(){const button=document.getElementById("s8Finalize");if(button)button.disabled=true;try{await flushSprint6AudioConsumptions();const runId=currentSprint6State?.state?.run_id;if(!runId)throw new Error("Finalization run identity is unavailable.");const pending=requestIdentity("s8-finalize",runId,{boundary:"act14",run_id:runId});await rpc("s8_finalize",{p_room_code:session.room_code,p_session_token:session.session_token,p_client_request_id:pending.requestId,p_expected_run_id:runId});clearRequestIdentity(pending.key);await refreshState()}catch(error){sprint3bStatus.innerHTML=`<span class="bad">${escapeHtml(error.message)}</span>`;if(button)button.disabled=false}}
function renderSprint8(payload){
  setSprint6CinematicMode({phase_key:"act14_complete"});
  choiceArea.classList.add("hidden");
  revealArea.classList.add("hidden");
  discussionPanel.classList.add("hidden");
  ...
  if(sprint8RevealRunId===payload.run_id)return;
  sprint8RevealRunId=payload.run_id;
  clearTimeout(sprint8RevealTimer);

  const previousExteriorSrc=document.getElementById("s6SceneImage")?.src||"";
  let stage=0;
  const stageNames=["fade","mission","escaped","remembered","they","end"];
  const frames=[
    `<div class="s8-exterior-stage"><img id="s8ExteriorImage" class="s8-exterior-image" ${previousExteriorSrc?`src="${escapeHtml(previousExteriorSrc)}"`:""} alt=""><div class="s8-fade" aria-hidden="true"></div></div>`,
    `<p>${localizedHtml("act14.001")}</p>`,
    `<p>${localizedHtml("act14.002")}</p>`,
    `<strong>${localizedHtml("act14.003")}</strong>`,
    `<strong class="s8-separate-screen">${localizedHtml("act14.004")}</strong>`,
    `<p>${localizedHtml("act14.005")}</p>`
  ],delays=[900,1100,1400,1800,1800];

  const advance=()=>{
    const fadeClass=stage===0?" s8-fade-stage":"";
    sprint3bText.innerHTML=`<section class="s8-ending${fadeClass} s8-stage-${stage}" data-s8-stage="${stageNames[stage]}" aria-live="polite">${frames[stage]}</section>`;
    if(stage===0&&!previousExteriorSrc)setS5Asset("s8ExteriorImage","ending.castle_exterior");
    if(stage<frames.length-1)sprint8RevealTimer=setTimeout(()=>{stage+=1;advance()},delays[stage]);
  };
  advance();
}
async function hydrateSprint6(payload){let scene=null;if(S6_SCENE_ASSETS.has(payload.scene_asset_key))scene=await setS5Asset("s6SceneImage",payload.scene_asset_key);if(scene&&payload.state.act_no===9)document.querySelectorAll(".s6-door[data-anchor]").forEach(node=>{const a=anchor(scene,node.dataset.anchor);if(!a){node.hidden=true;return}Object.assign(node.style,{left:`${a.x_percent}%`,top:`${a.y_percent}%`,width:`${a.width_percent}%`,height:`${a.height_percent}%`})});if(payload.state.phase_key==="act12_cinematic"&&payload.state.cinematic_stage===10){for(const audio of sprint6AudioHandles.values()){audio.pause();audio.currentTime=0;markSprint6AudioConsumed(audio.dataset.occurrenceId,"stopped")}sprint6AudioHandles.clear()}const key=payload.state.feedback_audio_key,identity=payload.audio_occurrence_id,locallyConsumed=Boolean(sprint6AudioOutbox()[identity]);await flushSprint6AudioConsumptions();if(!key||!identity||payload.audio_consumed||locallyConsumed||identity===sprint6AudioIdentity)return;sprint6AudioIdentity=identity;const asset=await rpc("asset_resolve",{p_asset_key:key}).catch(()=>null);if(!asset?.ok){await markSprint6AudioConsumed(identity,"stopped");return}if(!sprint6AudioArmed){sprint6PendingAudio={key,asset,occurrenceId:identity};return}await playSprint6Audio(key,asset,identity)}
async function sendSprint6Message(event){event.preventDefault();const input=document.getElementById("s6MessageText"),d=currentSprint6State.discussion,pending=requestIdentity("s6-message",d.discussion_session_id,{session:d.discussion_session_id,text:input.value});try{await rpc("s6_send_message_v2",{p_room_code:session.room_code,p_session_token:session.session_token,p_expected_discussion_session_id:d.discussion_session_id,p_client_request_id:pending.requestId,p_message_text:input.value});clearRequestIdentity(pending.key);await refreshState()}catch(error){sprint3bStatus.textContent=error.message}}
async function submitSprint6Station(event){event.preventDefault();const button=event.currentTarget.querySelector("button");button.dataset.s6Rpc="s6_complete_station_v2";button.dataset.s6Value=document.getElementById("s6StationValue").value;await runSprint6Action(button)}
async function runSprint6Action(button){button.disabled=true;const rpcName=button.dataset.s6Rpc,body={p_room_code:session.room_code,p_session_token:session.session_token,...sprint6Identity()};if(rpcName==="s6_close_discussion_v2")body.p_expected_discussion_session_id=button.dataset.s6Discussion;if(button.dataset.s6Choice)body[rpcName==="s6_submit_allocation_v2"?"p_role_key":"p_choice_id"]=button.dataset.s6Choice;if(button.dataset.s6Role)body.p_role_key=button.dataset.s6Role;if(button.dataset.s6Value)body.p_task_value=button.dataset.s6Value;const identity=`${body.p_expected_phase}.${body.p_expected_step}.${body.p_expected_round}.${button.dataset.s6Discussion||""}`,pending=requestIdentity(rpcName,identity,body);body.p_client_request_id=pending.requestId;try{await rpc(rpcName,body);clearRequestIdentity(pending.key);await refreshState()}catch(error){sprint3bStatus.innerHTML=`<span class="bad">${escapeHtml(error.message)}</span>`;button.disabled=false}}
