...
async function finalizeSprint8(){const button=document.getElementById("s8Finalize");if(button)button.disabled=true;try{await flushSprint6AudioConsumptions();const pending=requestIdentity("s8-finalize",session.room_code,{boundary:"act14"});await rpc("s8_finalize",{p_room_code:session.room_code,p_session_token:session.session_token,p_client_request_id:pending.requestId});clearRequestIdentity(pending.key);await refreshState()}catch(error){sprint3bStatus.innerHTML=`<span class="bad">${escapeHtml(error.message)}</span>`;if(button)button.disabled=false}}
