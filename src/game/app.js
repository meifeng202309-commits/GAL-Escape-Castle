...
const S6_SCENE_ASSETS=new Set(["shared.great_hall","shared.main_gate","ending.castle_exterior"]);
...
function sprint6Discussion(payload){const d=payload.discussion;if(!d)return"";const ready=Date.parse(d.phase_deadline)<=Date.now();return `<section class="discussion-room"><h3>${localizedHtml(d.phase_key==="act10_discussion"?"act10.004":d.phase_key==="act11_discussion"?"act11.001":"act09.004")}</h3><p class="muted">${escapeHtml(formatDeadline(d.phase_deadline,"discussion"))}</p><div class="transcript">${d.messages.length?d.messages.map(m=>`<article class="message"><b>${escapeHtml(m.display_name)}</b><p>${escapeHtml(m.message_text)}</p></article>`).join(""):localizedHtml("discussion.no_messages")}</div><form id="s6MessageForm" class="composer"><input id="s6MessageText" maxlength="1000" required><button>${localizedHtml("discussion.send_message")}</button></form><button data-s6-rpc="s6_close_discussion_guarded" data-s6-discussion="${d.discussion_session_id}" ${ready?"":"disabled"}>${localizedHtml("common.004")}</button></section>`}
function renderSprint6(payload){currentSprint6State=payload;const s=payload.state;choiceArea.classList.add("hidden");revealArea.classList.add("hidden");sprint3bPanel.classList.remove("hidden");const black=s.phase_key==="act12_cinematic"&&s.cinematic_stage>=8;let body=`<div class="s6-stage ${black?"blackout":""}"><p class="eyebrow">ACT ${s.act_no}</p>${payload.scene_asset_key?`<img id="s6SceneImage" class="s5-scene" alt="">`:""}${payload.private_clue?`<div class="notice">${localizedHtml(payload.private_clue.text_key)}</div>`:""}${localizedKeys(s.feedback_text_keys||[])}</div>`,actions="";if(payload.private_choices_revealed?.length)body+=`<div class="notice">${payload.private_choices_revealed.map(x=>`${escapeHtml(x.role_slot)}: ${localizedHtml(x.choice_id==="take"?"act10.004":"act10.005")}`).join("<br>")}</div>`;if(payload.discussion)actions=sprint6Discussion(payload);if(s.phase_key==="act9_console"){const choices=s.act9_step===1||s.act9_step===3?S6_DOORS:s.act9_step===2?[["enter_red","act09.015"],["do_not_enter","act09.016"]]:[["enter_blue","act09.020"],["stay","act09.021"]];actions=s5Buttons(choices,"s6_submit_group_choice_guarded").replaceAll("data-s5-","data-s6-")}if(s.phase_key==="act10_private")actions=s5Buttons([["take","act10.004"],["leave","act10.005"]],"s6_submit_private_choice_guarded").replaceAll("data-s5-","data-s6-");if(s.phase_key==="act10_final_vote")actions=s5Buttons([["take","act10.006"],["leave","act10.007"]],"s6_submit_group_choice_guarded").replaceAll("data-s5-","data-s6-");if(s.phase_key==="act10_result")actions=`<button data-s6-rpc="s6_advance_guarded">${localizedHtml("act10.010")}</button>`;if(s.phase_key==="act11_allocation")actions=s5Buttons((s.gold_key?[["A","act11.012"],["B","act11.013"],["WATCHER","act11.014"]]:[["A","act11.012"],["B","act11.013"],["C","act11.004"]]),"s6_submit_allocation_guarded").replaceAll("data-s5-","data-s6-");if(s.phase_key==="act12_tasks"&&payload.allocation){const task={A:["act11.005","1897"],B:["act11.007","lever_center"],C:["act11.008","silver_key_inserted"],WATCHER:["act11.015","corridor_watched"]}[payload.allocation];actions=payload.station_task?`<button data-s6-rpc="s6_engage_guarded" data-s6-role="${escapeHtml(payload.allocation)}">${localizedHtml(payload.allocation==="WATCHER"?"act12.004":"act12.001")}</button>`:`${localizedHtml(task[0])}${payload.allocation==="A"?`<form id="s6StationForm" class="composer"><input id="s6StationValue" inputmode="numeric" maxlength="4" required><button>${localizedHtml("act12.001")}</button></form>`:`<button data-s6-rpc="s6_complete_station_guarded" data-s6-value="${task[1]}">${localizedHtml(payload.allocation==="B"?"act12.002":payload.allocation==="C"?"act12.003":"act12.004")}</button>`}`}if(s.phase_key==="act12_pressure"){const failing=payload.allocation===s.mechanism_failure_station;actions=s5Buttons(failing?[["one","act12.008"],["two","act12.009"],["three","act12.010"],["four","act12.011"]]:[["hold","act12.012"],["encourage","act12.013"],["watch","act12.014"],["prepare","act12.015"]],"s6_submit_private_choice_guarded").replaceAll("data-s5-","data-s6-")}if(s.phase_key==="act13_escape")actions=`<button data-s6-rpc="s6_advance_guarded">${localizedHtml("common.004")}</button>`;sprint3bText.innerHTML=body;sprint3bActions.innerHTML=actions;hydrateSprint6(payload);sprint3bActions.querySelectorAll("[data-s6-rpc]").forEach(b=>b.addEventListener("click",()=>runSprint6Action(b)));document.getElementById("s6MessageForm")?.addEventListener("submit",sendSprint6Message);document.getElementById("s6StationForm")?.addEventListener("submit",submitSprint6Station)}
async function hydrateSprint6(payload){if(S6_SCENE_ASSETS.has(payload.scene_asset_key))await setS5Asset("s6SceneImage",payload.scene_asset_key);const key=payload.state.feedback_audio_key,identity=`${payload.state.phase_key}.${payload.state.cinematic_stage}.${key}`;if(!key||identity===sprint6AudioIdentity)return;sprint6AudioIdentity=identity;const asset=await rpc("asset_resolve",{p_asset_key:key}).catch(()=>null);if(!asset?.ok){sprint3bStatus.textContent=`ASSET_UNAVAILABLE · ${key}`;return}new Audio(`https://qdcbdcjobzytzhnhfwyn.supabase.co/storage/v1/object/public/${asset.storage_path}`).play().catch(()=>{})}
async function sendSprint6Message(event){event.preventDefault();const input=document.getElementById("s6MessageText"),d=currentSprint6State.discussion;try{await rpc("s6_send_message_guarded",{p_room_code:session.room_code,p_session_token:session.session_token,p_expected_discussion_session_id:d.discussion_session_id,p_client_request_id:crypto.randomUUID(),p_message_text:input.value});await refreshState()}catch(error){sprint3bStatus.textContent=error.message}}
async function submitSprint6Station(event){event.preventDefault();const button=event.currentTarget.querySelector("button");button.dataset.s6Rpc="s6_complete_station_guarded";button.dataset.s6Value=document.getElementById("s6StationValue").value;await runSprint6Action(button)}
async function runSprint6Action(button){button.disabled=true;const rpcName=button.dataset.s6Rpc,body={p_room_code:session.room_code,p_session_token:session.session_token,...sprint6Identity()};if(rpcName==="s6_close_discussion_guarded")body.p_expected_discussion_session_id=button.dataset.s6Discussion;else{body.p_client_request_id=crypto.randomUUID();if(button.dataset.s6Choice)body[rpcName==="s6_submit_allocation_guarded"?"p_role_key":"p_choice_id"]=button.dataset.s6Choice;if(button.dataset.s6Role)body.p_role_key=button.dataset.s6Role;if(button.dataset.s6Value)body.p_task_value=button.dataset.s6Value}try{await rpc(rpcName,body);await refreshState()}catch(error){sprint3bStatus.innerHTML=`<span class="bad">${escapeHtml(error.message)}</span>`;button.disabled=false}}
