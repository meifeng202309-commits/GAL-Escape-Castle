...
function renderSprint6(payload){currentSprint6State=payload;const s=payload.state;choiceArea.classList.add("hidden");revealArea.classList.add("hidden");sprint3bPanel.classList.remove("hidden");let body=`<p class="eyebrow">ACT ${s.act_no}</p>${payload.private_clue?`<div class="notice">${localizedHtml(payload.private_clue.text_key)}</div>`:""}`,actions="";if(payload.private_choices_revealed?.length)body+=`<div class="notice">${payload.private_choices_revealed.map(x=>`${escapeHtml(x.role_slot)}: ${escapeHtml(x.choice_id)}`).join("<br>")}</div>`;if(s.phase_key==="act9_discussion")actions=`<button data-s6-rpc="s6_open_act9_console">${localizedHtml("common.004")}</button>`;if(s.phase_key==="act9_console"){const choices=s.act9_step===1||s.act9_step===3?S6_DOORS:s.act9_step===2?[["enter_red","act09.015"],["do_not_enter","act09.016"]]:[["enter_blue","act09.020"],["stay","act09.021"]];actions=s5Buttons(choices,"s6_submit_group_choice").replaceAll("data-s5-","data-s6-")}if(s.phase_key==="act10_private")actions=s5Buttons([["take","act10.004"],["leave","act10.005"]],"s6_submit_private_choice").replaceAll("data-s5-","data-s6-");if(s.phase_key==="act10_final_vote")actions=s5Buttons([["take","act10.006"],["leave","act10.007"]],"s6_submit_group_choice").replaceAll("data-s5-","data-s6-");if(s.phase_key==="act10_result")actions=`<button data-s6-rpc="s6_advance">${localizedHtml("act10.010")}</button>`;if(s.phase_key==="act11_allocation")actions=s5Buttons((s.gold_key?[["A","act11.012"],["B","act11.013"],["WATCHER","act11.014"]]:[["A","act11.012"],["B","act11.013"],["C","act11.004"]]),"s6_submit_allocation").replaceAll("data-s5-","data-s6-");if(s.phase_key==="act12_engage"&&payload.allocation)actions=`<button data-s6-rpc="s6_engage" data-s6-role="${escapeHtml(payload.allocation)}">${localizedHtml(payload.allocation==="WATCHER"?"act12.004":"act12.001")}</button>`;if(s.phase_key==="act12_pressure"){const failing=payload.allocation===s.mechanism_failure_station;actions=s5Buttons(failing?[["one","act12.008"],["two","act12.009"],["three","act12.010"],["four","act12.011"]]:[["hold","act12.012"],["encourage","act12.013"],["watch","act12.014"],["prepare","act12.015"]],"s6_submit_private_choice").replaceAll("data-s5-","data-s6-")}if(s.act_no===13)body+=localizedKeys(["act12.027","act13.001","act13.002","act13.003"]);sprint3bText.innerHTML=body;sprint3bActions.innerHTML=actions;sprint3bActions.querySelectorAll("[data-s6-rpc]").forEach(b=>b.addEventListener("click",()=>runSprint6Action(b)))}
async function runSprint6Action(button){button.disabled=true;const body={p_room_code:session.room_code,p_session_token:session.session_token};if(button.dataset.s6Choice){body[button.dataset.s6Rpc==="s6_submit_allocation"?"p_role_key":"p_choice_id"]=button.dataset.s6Choice;body.p_client_request_id=crypto.randomUUID()}if(button.dataset.s6Role)body.p_role_key=button.dataset.s6Role;try{await rpc(button.dataset.s6Rpc,body);await refreshState()}catch(error){sprint3bStatus.innerHTML=`<span class="bad">${escapeHtml(error.message)}</span>`;button.disabled=false}}
