# Inter-Agent Talk Protocol V4

Project: GAL Escape Castle  
Protocol version: V4  
Status: ACTIVE  
Supersedes: Inter-Agent Talk Protocol V3  
...
| VA | Visual Agent | Primary: visual production/continuity. Auxiliary: explicitly allocated bounded fragmented/temporary non-authoritative production-support work when semantics are fixed and no conflicting canonical owner exists |
...
Auxiliary ownership may be explicitly allocated for bounded fragmented / temporary production-support work when:
- semantic/product requirements are already fixed;
- no conflicting canonical owner exists;
- the work is non-runtime-authoritative;
- it does not change gameplay, database, security, lifecycle, localization authority, asset identity, approval authority, or ACTIVE publication authority.

Sprint9 audio-candidate creation / legal sourcing for the six existing canonical audio keys is an approved example of this auxiliary lane.

VA must not silently alter gameplay logic or acquire runtime publication/ACTIVE authority through an auxiliary assignment.
...
`ALL` is an **exceptional receiver**, not a routine FYI channel.
...
only when the current decision / rule / gate **materially affects every active project role**, or when coordinated action is genuinely required from all active roles.
...
If one or more active roles are not materially involved, do **not** use `ALL`. Send only the minimum necessary targeted message(s).

Example of valid `ALL` use:
- a new inter-Agent communication protocol that changes how every Agent must communicate.

Examples of invalid `ALL` use:
- a CD/CA audit result that does not change VA/GA/ISA work;
- a VA asset-production detail that only CD must integrate;
- a message sent merely so unrelated Agents are “aware”.

---

# 7A. Minimum Necessary Recipient Rule — NO FYI

Formal inter-Agent communication must use the **smallest recipient set necessary for the current decision and its direct consequences**.

An Agent should receive a message only if at least one of the following is true:

1. the Agent has authority required to make / clarify the current decision;
2. the Agent must perform a concrete next action because of the decision;
3. the decision materially changes that Agent's current ownership, scope, interface, gate, canonical obligation, or active work;
4. the Agent must independently audit / verify the specific result as the next workflow step.

The following are **not** sufficient reasons to send a message:

- “for awareness”;
- “FYI”;
- courtesy copying;
- keeping an Agent “in the loop”;
- possible future relevance;
- the Agent works on the same project;
- traceability alone.

Traceability belongs in canonical files, Action Logs, CURRENT STATUS, audit artifacts and existing `agent-comms` history. It does not justify extra notification traffic.

Before sending, the sender must ask:

```text
Does this Agent influence the current decision?
OR
Is this Agent's current authorized work materially changed by the decision?
OR
Is this Agent the required next executor/auditor?
```

If all answers are NO, do not send that Agent the message.

After a decision is resolved, send the disposition only to the Agents whose next action / scope / authority is affected.

Do not relay another Agent's message onward merely for visibility. Relay only when the content becomes materially relevant to the new receiver's current work.
...
11. Apply the Minimum Necessary Recipient Rule; do not create FYI-only communication.
...
Use only when **every active role is materially affected** by the audit/governance outcome or every role must change behavior.

Do not use `CA → ALL` merely because a report is important.

For a cross-system issue affecting only a subset of roles, send targeted messages to that subset instead.
...
This file preserves the five-role protocol and bounded VA auxiliary responsibility, and adds the Minimum Necessary Recipient / NO-FYI communication rule. It supersedes V3.
